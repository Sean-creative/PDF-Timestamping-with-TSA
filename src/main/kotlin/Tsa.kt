...
class Tsa {
    ...
    private fun getTimeStampToken(signature: ByteArray?): ByteArray { //만약 이전에 다른 데이터에 대한 해시를 계산했다면, 그 상태를 초기화하여 새로운 데이터에 대한 해시를 계산할 수 있도록함
        digest.reset() //signature 바이트 배열에 대한 해시 값을 계산
        val hash = digest.digest(signature)

        // 암호학적으로 안전한 난수를 생성
        val random = SecureRandom() // 32-bit 난수 생성
        val nonce = random.nextInt() // nonce 변수에 저장된 값은 나중에 Timestamp Token을 생성할 때 서버에 의해 생성된 고유한 값으로 사용될 수 있습니다.
        ...
        val timeStampRequestGenerator = TimeStampRequestGenerator() // 인증서를 요청하도록 설정
        timeStampRequestGenerator.setCertReq(true)

        // 현재 해시 알고리즘에 해당하는 ASN.1 OID (Object Identifier)를 가져옴
        val oid = getHashObjectIdentifier(digest.algorithm) // timeStampRequestGenerator에게 전송할 Timestamp Token 요청을 생성합니다.
        ...
        val signerInfoGenerator: SignerInfoGenerator = JcaSimpleSignerInfoGeneratorBuilder().build(
            "SHA256WithRSAEncryption", PdfSign.cert.privateKey, PdfSign.cert.certificate as X509Certificate
        ...
        val certs = JcaCertStore(certList) //서명 시간 정보를 담은 타임스탬프 토큰에 추가적인 보안 관련 정보를 첨부
    ...
    private fun signTimeStamp(signer: SignerInformation): SignerInformation { // ASN1은 데이터 구조와 표현을 기술하기 위한 표준
        ...
        timeStampToken = getTimeStampToken(signer.signature) // Timestamp Token을 나타내는 ASN1 객체 식별자(OID)를 설정
        val oid = PKCSObjectIdentifiers.id_aa_signatureTimeStampToken // ASN1 구조를 이용하여 Timestamp Token을 포함하는 Attribute 객체를 생성
        // oid는 Timestamp Token을 식별하는데 사용되며, DERSet을 이용하여 token을 ASN.1으로 변환한 후 Attribute에 추가
        val signatureTimeStamp: ASN1Encodable = Attribute(oid, DERSet(ASN1Primitive.fromByteArray(timeStampToken))) // 앞에서 생성한 Attribute 객체를 vector에 추가
    ...
    fun signTimeStamps(cmsSignedData: CMSSignedData): CMSSignedData { //각 서명자(SignerInformation)에 대한 정보를 담고 있는 SignerInformationStore를 가져옵니다.
