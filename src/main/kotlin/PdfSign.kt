...
object PdfSign {
    ...
    private fun cert(p12File: String, password: String, isUrl: Boolean = false): Cert? {
        val pwCharArray = password.toCharArray() //KeyStore는 Java에서 암호화된 개인 키, 디지털 인증서 및 인증서 체인을 저장하는 데 사용되는 클래스
        ...
        val aliases = keystore.aliases() //단 여러개의 별칭이 있어도 첫번째 별칭만 처리함
    ...
    private fun signByteArray(param: Param): ByteArray? {
        val calendar = Calendar.getInstance(TimeZone.getTimeZone("UTC")) // 날짜가 주어진 경우 UTC로 변환하여 설정
        param.date?.also {
            val d = it.split(' ')
            if (d.size == 2) {
    ...
    // main 함수
    @JvmStatic
    fun main(args: Array<String>) { // PFX 파일을 기반으로 Cert 객체 생성
        ...
        try { // 현재 UTC 시간을 가져오기
            ...
            signByteArray( // date는 원하는 시간을 하드코딩 해도 됩니다. ex) 2023-12-07 01:22:22
                ...
                Param(File("sample.pdf").readBytes(), cert, formattedUTCTime)
            )?.also { // 서명된 PDF 파일을 새로운 파일로 저장
                File("sample_signed.pdf").writeBytes(it)
            } ?: error("file error")
        } catch (e: Throwable) { // 예외 발생 시 에러 메시지 출력
